...
# --- 자동 로그인 설정 ---
AUTO_LOGIN = True   # 로그인 화면이 뜨면 '다음→로그인' 을 자동으로 눌러 로그인 시도
LOGIN_PW   = ""     # 비워두면 브라우저에 저장된 비밀번호에 의존.
                    # 자동 로그인이 안 되면 여기에 비밀번호를 넣으면 직접 입력해 로그인함.
LOGIN_ALERT_GRACE = 300   # 로그인 풀린 뒤 이 시간(초) 동안 복구 안 되면 '로그인 필요' 메일 발송.
                          # 300=5분. 그 안에 복구되면 메일 안 보냄.

# --------------------------- 메일(SMTP) 설정 ---------------------------
MAIL_FROM = "sjlee@hanbalmasstech.com"     # 보내는 사람
# 받는 사람 — 여러 명이면 아래처럼 줄줄이 추가 (도메인 전체 발송은 불가, 주소를 나열해야 함)
MAIL_TO = [
    "sjlee@hanbalmasstech.com",
    # "hkpark@hanbalmasstech.com",
    # "jhlee@hanbalmasstech.com",
    # "hbjeong@hanbalmasstech.com",
    # "hemun@hanbalmasstech.com",
    # "jhmin@hanbalmasstech.com",
]
...
def _start_stop_listener():
    """콘솔에서 'stop'(또는 q/quit/종료) 입력 시 감시를 안전하게 종료.
       정상 종료가 지연되면(브라우저 드라이버 정리 멈춤 등) 강제 종료한다."""
    def listen():
        global _stop_requested
        try:
            for line in sys.stdin:
                if line.strip().lower() in ("stop", "quit", "q", "종료"):
                    _stop_requested = True
                    log("중지 요청 받음 — 정리 후 종료합니다...")
                    time.sleep(5)          # 정상 종료(크롬 닫기 등)에 줄 시간
                    log("종료합니다.")
                    os._exit(0)            # 정리가 멈춰 있어도 확실히 종료
        except Exception:
            pass
...
def _interruptible_sleep(seconds):
    """중지 요청이 오면 즉시 깨는 sleep(1초 단위 확인)."""
...
def _recipient_list():
    """MAIL_TO 를 주소 리스트로 정규화. 문자열 한 명 / 리스트 여러 명 둘 다 허용."""
    if isinstance(MAIL_TO, str):
        return [MAIL_TO] if MAIL_TO else []
    return [a.strip() for a in MAIL_TO if a and a.strip()]


def _recipients_text():
    """로그 표시용 수신자 문자열."""
    return ", ".join(_recipient_list()) or "(수신자 없음)"
...
def _focus_first_text_input(page) -> bool:
    """로그인 입력칸에 포커스(JS). '아이디' 칸 우선, 비활성/체크박스는 제외.
       (회사코드 칸은 disabled 라 클릭이 안 되므로 JS focus 로 처리)"""
    js = """() => {
      const skip = ['checkbox','radio','hidden','button','submit'];
      const els = Array.from(document.querySelectorAll('input')).filter(e =>
        e.offsetParent !== null && !e.disabled && !skip.includes(e.type));
      if (!els.length) return false;
      let i = els.findIndex(e => (e.placeholder||'').includes('아이디'));
      if (i < 0) i = els.findIndex(e => e.type === 'password');
      if (i < 0) i = 0;
      els[i].focus();
      return true;
    }"""
...
def try_auto_login(page) -> bool:
    """사용자가 확인한 방식 그대로: 아이디 칸에 포커스 후 Enter 두 번.
       (화면1 제출 → 화면2 로 전환 → 화면2 제출)
       LOGIN_PW 가 설정돼 있으면 화면2 에서 비밀번호를 직접 입력."""
    try:
        # 화면1: 아이디 칸 포커스 → Enter
        if not _focus_first_text_input(page):
            return False
        page.keyboard.press("Enter")
        page.wait_for_timeout(1500)        # 화면 전환 대기
        ...
        # 화면2: 다시 포커스 → Enter (로그인 제출)
        _focus_first_text_input(page)
        page.keyboard.press("Enter")
        page.wait_for_timeout(250)        # 로그인 처리 대기
        return True
    except Exception as e:
        log(f"자동 로그인 입력 오류: {e}")
        return False


def _ensure_logged_in(page) -> bool:
    """로그인 화면이면 자동 로그인을 반복 시도하고, 유예시간 지나면 메일 알림.
       로그인되면 True, 중지 요청이면 False."""
    if not _is_login_page(page):
        return True
    lost_since = time.time()
    alerted = False
    log(f"로그인 필요 — 자동 로그인 시도 시작. ({LOGIN_ALERT_GRACE // 60}분 내 복구 안 되면 메일)")
    while not _stop_requested and _is_login_page(page):
        if AUTO_LOGIN:
            try:
                try_auto_login(page)
            except Exception as e:
            ...
            if not _is_login_page(page):
                log("자동 로그인 성공.")
                return True
        waited = time.time() - lost_since
        if (not alerted) and (waited >= LOGIN_ALERT_GRACE):
            log("로그인 필요 — 유예시간 경과, 알림 메일 발송.")
            try:
            ...
            log(f"로그인 대기 중... (경과 {int(waited)}초 / 유예 {LOGIN_ALERT_GRACE}초)")
        _interruptible_sleep(3)
    return not _is_login_page(page)


def run_watch():
    """감시 모드: 크롬을 띄워 로그인 확보 후 1분마다 목록 확인.
       로그인이 풀리면 크롬을 닫고 새로 띄워 다시 로그인한다.
       (이 그룹웨어는 세션 쿠키를 써서, 창을 닫으면 로그인이 풀림)"""
    from playwright.sync_api import sync_playwright
    state = load_state()
    log(f"감시 시작 (주기 {POLL_SECONDS}초, 수신자 {_recipients_text()}).")
    log("※ 종료하려면 이 창에 'stop' 입력 후 Enter (또는 창을 닫으세요).")
...
def run_test():
    ...
    try:
        ...
        return
    log(f"테스트 메일을 {_recipients_text()} 로 보냈습니다. 받은편지함을 확인하세요.")
